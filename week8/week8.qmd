...
# plotRGB(landsat_cropped, r = 3, g = 2, b = 1, stretch = "lin")

# Mask the raster to southern portion of SB county
landsat_masked <- mask(landsat_cropped, SB_county_south)

# Remove unnecessary object from env
...
landsat <- (landsat * 2.75e-05 - 0.2) * 100
...
training_data_values <- terra::extract(landsat, training_data, 
                                       df = TRUE) # Return values as df rather than matrix
...
```{r}
# Classify image based on descision tree
...
# Create empty raster with the same dimensions as our original landsat data
...
levels(SB_classification_kmeans) <- data.frame(
  ID = 1:4,
  landcover = c("soil/dead grass", "urban", "green vegitation", "water"))
```
