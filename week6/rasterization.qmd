...
if(crs(elevation) == crs(boundary)) {
  print("Coordiate reference systems match")
} else {
...
template_raster <- terra::rast(ext(cycle_hire_osm_projected), resolution = 1000, crs = crs(cycle_hire_osm_projected))
```
...
# Define NDVI as the normalized difference between NIR and red 
...
The normalized moisture index: vegetation index used in remote sensing used in RS to measure and monitor the water content in vegitation
