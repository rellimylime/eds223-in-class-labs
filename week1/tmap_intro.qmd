...
tm_shape(world_vector) + # defines the spatial object to plot
  tm_polygons() +        # how to plot the object
tm_shape(world_cities) + # defines the second spatial object to plot
...
```{r}
# plot vector and raster objects together
...
```{r}
# Because it assumes the extent and the CRS of the first layer
tm_shape(world_elevation) +
  tm_raster() +
tm_shape(world_vector) +
  tm_borders() +
tm_shape(world_cities, is.main = TRUE) + # Plotting the world_cities last but set as main shape (still want the extent or crs that world cities (ELEVATION?) is but want it to be on top)
  tm_dots() +
  tm_text("name")
```
...
tm_shape(world_vector) +
  tm_polygons(fill = "life_expectancy",
              fill.scale = tm_scale(values = c("pink", "purple")),
              fill.legend = tm_legend(title = "Life Expectancy (years)",
              posistion = tm_pos_out("right")))
...
tm_shape(world_vector) +
  tm_polygons(fill = "life_expectancy", 
              fill.scale = tm_scale(values = "brewer.yl_gn"), #TlOrBr, Pastel1, Accent are examples of other palettes
              fill.legend = tm_legend(title = "Life Expectancy (years)"))
```
