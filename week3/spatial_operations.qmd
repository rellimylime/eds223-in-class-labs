...
tm_layout(title = "Bracket subsetting", meta.margins = c(.2, 0, 0, 0))
...
tm_layout(title = "st_filter()", meta.margins = c(.2, 0, 0, 0))
...
tm_shape(c_height3) +
  tm_dots(fill = "red") +
#tm_title() +
tm_layout(title = "st_intersects()", meta.margins = c(.2, 0, 0, 0))
...
joined_df <-left_join(df_A, df_B, join = st_...)
...
if(any(st_intersects(cycle_hire, cycle_hire_osm, sparse = FALSE)) == TRUE) { # Sparse makes the return logical
...
# Check results
if (nrow(cycle_hire) == nrow(cycle_hire_join)) {
  print("join matches original data dimsensions")
} else {
...
map1 <- tm_shape(nz_elevation) +
  ...
  tm_layout(title = "group_by()", meta.margins = c(.2, 0, 0, 0))

map2 <- tm_shape(nz_elevation) +
  tm_polygons(fill = "elevation",
              fill.legend = tm_legend(title = "Mean elevation (meters)")) +
  tm_layout(title = "aggregate()", meta.margins = c(.2, 0, 0, 0))
...
map1 <- tm_shape(us_states) +
  ...
  tm_layout(title = "US States", meta.margins = c(.2, 0, 0, 0))
...
map1 <- tm_shape(us_west) +
  ...
  tm_layout(title = "western states", meta.margins = c(.2, 0, 0, 0))

map2 <- tm_shape(us_west_union) +
  tm_polygons() +
  #tm_title(text = "western states union")+ 
  tm_layout(title = "western states union", meta.margins = c(.2, 0, 0, 0))

map3 <- tm_shape(texas) +
  tm_polygons() +
  #tm_title(text = "TX")+ 
  tm_layout(title = "TX", meta.margins = c(.2, 0, 0, 0))

map4 <- tm_shape(texas_union) +
  tm_polygons() +
  #tm_title(text = "TX + western states union")+ 
  tm_layout(title = "TX + western states union", meta.margins = c(.2, 0, 0, 0))
...
map1 <- tm_shape(seine_buffer_5km) +
  ...
  tm_layout(title = "5km buffer", meta.margins = c(.2, 0, 0, 0))
...
map2 <- tm_shape(seine_buffer_50km) +
  ...
  tm_layout(title = "50km buffer", meta.margins = c(.2, 0, 0, 0))
...
map2 <- map1 +
  ...
  tm_fill(col = "purple") +
  tm_layout(title = "st_intersection()", meta.margins = c(.2, 0, 0, 0))

map3 <- map1 +
  tm_shape(difference_x_y, bbox = bbox) +
  tm_fill(col = "purple") +
  tm_layout(title = "st_difference(x,y)", meta.margins = c(.2, 0, 0, 0))


map4 <- map1 +
  tm_shape(difference_y_x, bbox = bbox) +
  tm_fill(col = "purple") +
  tm_layout(title = "st_difference(y,x)", meta.margins = c(.2, 0, 0, 0))

map5 <- map1 +
  tm_shape(union, bbox = bbox) +
  tm_fill(col = "purple") +
  tm_layout(title = "st_union()", meta.margins = c(.2, 0, 0, 0))

map6 <- map1 +
  tm_shape(sym_difference, bbox = bbox) +
  tm_fill(col = "purple") +
  tm_layout(title = "st_sym_diffference()", meta.margins = c(.2, 0, 0, 0))
...
# second appraoch: st_filter()
p_xy2 <- p %>% 
  st_filter(., x_and_y)

# thrid approach: st_intersection()
p_xy3 = st_intersection(p, x_and_y)
```

```{r}
# Map 2: original points only
map2 <- map1 +
  tm_shape(p) +
  tm_dots(alpha = 0.5) +
  tm_layout(title = "original")

# Map 3: just the base map + filtered points (bracket subsetting)
map3 <- map1 +
  tm_shape(p_xy1) +
  tm_symbols(col = "purple", size = 0.2) +
  tm_layout(title = "bracket subsetting")

# Map 4: just the base map + filtered points (st_filter)
map4 <- map1 +
  tm_shape(p_xy2) +
  tm_symbols(col = "purple", size = 0.2) +
  tm_layout(title = "st_filter()")

# Map 5: just the base map + filtered points (st_intersection)
map5 <- map1 +
  tm_shape(p_xy3) +
  tm_symbols(col = "purple", size = 0.2) +
...
map1 <- tm_shape(seine) +
  tm_lines() +
  tm_layout(title = "original", meta.margins = c(.2, 0, 0, 0))

map2 <- tm_shape(seine_simple) +
  tm_lines() +
  tm_layout(title = "st_simplify()", meta.margins = c(.2, 0, 0, 0))
